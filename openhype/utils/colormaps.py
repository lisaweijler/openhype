...
import openhype.utils.colors as colors
...
def apply_pca_colormap(
    ...
    global_max=None,
    global_min=None,  # , #global_max=0.3, global_min=-0.3
):
    ...
    s = d / mdev
    m = 3.0  # 3.0  # this is a hyperparam controlling how many std dev outside for outliers
    # rins = image[s[:, 0] < m, 0]
    # gins = image[s[:, 1] < m, 1]
    # bins = image[s[:, 2] < m, 2]
    rins = image[:, 0]
    ...
    print(rins.min(), gins.min(), bins.min())
    print(rins.max(), gins.max(), bins.max())
    if global_max is None or global_min is None:
        ...
        image = torch.clamp((image - global_min) / (global_max - global_min), 0, 1)
    image_long = (image * 255).long()
    image_long_min = torch.min(image_long)
    image_long_max = torch.max(image_long)
    assert image_long_min >= 0, f"the min value is {image_long_min}"
