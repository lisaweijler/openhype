...
import cv2
from openhype.utils.colormaps import apply_colormap
...
def smooth(mask):
    ...
    for i in range(h):
        for j in range(w):
            square = mask[
                max(0, i - scale) : min(i + scale + 1, h - 1),
...
def colormap_saving(image: torch.Tensor, colormap_options, save_path, mask=None):
    """
    ...
    if mask is not None:
        mask = mask.cpu().numpy()  # Make sure mask is a numpy array too
        if mask.shape != output_image.shape[:2]:
            raise ValueError(
                f"Mask shape {mask.shape} does not match image shape {output_image.shape[:2]}"
            )

        # Set the masked pixels to white
        output_image[mask] = [255, 255, 255]  # Assuming output_image is (H, W, 3) RGB

    if save_path is not None:
        media.write_image(save_path.with_suffix(".png"), output_image, fmt="png")
        media.write_image(save_path.with_suffix(".pdf"), output_image, fmt="pdf")
